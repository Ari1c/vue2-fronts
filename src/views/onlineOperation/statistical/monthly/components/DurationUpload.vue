   ...
   <div>
        ...
        <div class="data-view mt15 flex">
          ...
          <div class="chart-box wp25">
            <div class="tit flex align-center">
              <img src="@/assets/imgs/online/monthly/tit.png" />
              <span>接入和落地近6个月时长</span>
            </div>
            <div id="durationChart3" ref="durationChart3" class="durationChart" v-loading='loading3' element-loading-background = "rgba(0,0,0,0)"></div>
          </div>
          <div class="chart-box wp25">
            <div class="tit flex align-center">
              <img src="@/assets/imgs/online/monthly/tit.png" />
              <span>链路贯通近6个月时长</span>
            </div>
            <div id="durationChart4" ref="durationChart4" class="durationChart" v-loading='loading3' element-loading-background = "rgba(0,0,0,0)"></div>
          </div>
        </div>
    </div>
</template>
...
<script>
const imgList = [
    {img:require('@/assets/imgs/online/monthly/viewIcon_11.png'),tit:'上传需求数:',num:'',unit:'个',color:'colorB',isJump:true},
    {img:require('@/assets/imgs/online/monthly/viewIcon_12.png'),tit:'上传数据表数:',num:'',unit:'个',color:'colorB',isJump:false},
    {img:require('@/assets/imgs/online/monthly/viewIcon_13.png'),tit:'数据上传未完成数:',num:'',unit:'个',color:'colorR',isJump:true},
    {img:require('@/assets/imgs/online/monthly/viewIcon_14.png'),tit:'数据上传平均时长:',num:'',unit:'天',color:'colorB',isJump:false},
    ];
import {setTaskDuration,setTrend1,setDurationLine,setDurationBar}  from '@/utils/monthlyEcharts'
    ...
    jump(i){
     ...
     setTaskChart(data) {
      ...
      setDurationChart3(data) {
       ...
       setDurationChart4(data) {
        let width = 12;
        let option = setDurationBar(width,data);
        this.durationChart4.clear();
        this.durationChart4.setOption(option);
      },
      chartResize(){
        ...
        if(this.durationChart4){this.durationChart4.resize()};
      },
      // 获取指标总览
      getDataView(){
        https.queryOverView(0).then(res =>{
          this.imgList[0].num = res.data.allTotal;
          this.imgList[1].num = res.data.tableCount;
          this.imgList[2].num = res.data.incompleteTotal;
          this.imgList[3].num = res.data.avgDays;
        })
      },
       // 获取任务总时长
      getTaskLinkDuration(){
        this.loading1 = true;
        https.queryTaskLinkDuration(0).then(res =>{
          this.loading1 = false;
          this.$nextTick(() =>{
      ...
      getTrendAnalysis(){
        this.loading2 = true;
        https.queryTrendAnalysis(0).then(res =>{
          this.loading2 = false;
           this.$nextTick(() =>{
      ...
      getStepsAnalysis(){
         this.loading3 = true;
        https.queryStepsAnalysis(0).then(res =>{
          this.loading3 = false;
          var list = {
          ...
          var list1 = {
            xdata:res.data.monthList,
            ydata:res.data.ydsyList
          }
           var list2 = {
            xdata:res.data.monthList,
            ydata:res.data.sjjrList
          }
           var list3 = {
            xdata:res.data.monthList,
            ydata:res.data.llgtList
          }
          this.$nextTick(() =>{
            ...
            this.durationChart4 = this.$echarts.init(this.$refs.durationChart4);
            this.setDurationChart4(list3);
          })
        }).catch(error =>{
          this.loading3 = false;
        })
